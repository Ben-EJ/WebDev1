...
<html>
  <head>
    ...
    <link rel="stylesheet" href="/map/maps.css">
  </head>
    <script>
        let map;
                    ...
                    for(var x = 0; x < recElements.length; x++){
                        ...
                        if(dateTime[2] == valueYear && dateTime[1] == valueMonth && dateTime[0] == valueDay){
                            if(dateTime[3].toString().trim() == time.trim()){
                                ...
                                if(pollutant.trim() == "nox"){
                                    var thePolNOX = parseInt(recElements[x].getAttribute('nox'));
                                    if(thePolNOX == null){
                                        console.log(thePolNOX);
                                    }else{
                                        generateMarker(map, { lat: parseFloat(array[0]), lng: parseFloat(array[1]) }, thePolNOX, colourLevel(thePolNOX));
                                        console.log("Found");
                                    }
                                }
                                else if(pollutant.trim() == "no"){
                                    var thePolNO = parseInt(recElements[x].getAttribute('no'));
                                    if(thePolNO == null){
                                        console.log(thePolNO);
                                    }else{
                                        generateMarker(map, { lat: parseFloat(array[0]), lng: parseFloat(array[1]) }, thePolNO, colourLevel(thePolNO));
                                        console.log("Found");
                                    }
                                }
                                else if(pollutant.trim() == "no2"){
                                    var thePolNO2 = parseInt(recElements[x].getAttribute('no2'));
                                    if(thePolNO2 == null){
                                        console.log(thePolNO2);
                                    }else{
                                        generateMarker(map, { lat: parseFloat(array[0]), lng: parseFloat(array[1]) }, thePolNO2, colourLevel(thePolNO2));
                                        console.log("Found");
                                    }
                                }
                            }                    
...
<select name="time" id="time">
    ...
    <button onclick="initMap()">Generate Map(Heat Map Markers)</button>
    <button onclick="initMapColourEncodings()">Generate Map(Colour Encodings)</button>
    
    <div id="map"></div>
    <div class="ColourTable"><table id="colourTable"></table></div>
    </div>
    <script async
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBy3DhKi_LhOqHmhS-HEbtw6nO63vt5HDc&libraries=visualization&callback=emptyMap">
    </script>
  </body>
</html>